...
It has a simple zero at each lattice point, and no poles since it converges absolutely. Observe that this function is *odd*, meaning that $\sigma(-z;\Lambda)=-\sigma(z;\Lambda)$, which follows from $-\Lambda^{\times} = \Lambda^{\times}$. 
...
in $\mathbb{C}$, the functions (omitting $\Lambda$ from notation for convenience)
