...
I recently spent some time studying the Weil pairing and was encouraged to record and share some notes and reflections. The goal of this post is to explore its complex-analytic perspective, which will allow us to construct the pairing very explicitly. A more detailed overview is given at the end of the next section.

## The Weil Pairing over Fields

Before specialising to the complex-analytic setting, we will first have to review a standard definition of the Weil pairing, for which we need to recall some concepts: rational functions on an elliptic curve, their zeroes and poles, divisors, and linear equivalence.
...
Due to the presence of poles (coming from zeroes in $G$), the function $f$ typically does not extend to all of $E^\mathrm{aff}$. [Hilbert's Nullstellensatz](https://en.wikipedia.org/wiki/Hilbert%27s_Nullstellensatz) implies that $G$ vanishes identically on $E^\mathrm{aff}$ if and only if $G$ lies in the ideal generated by the Weierstrass equation defining $E$. 

The number of zeroes and poles of $f$ on a smooth projective curve like $E$ (or on a compact Riemann surface) turns out to be finite, and the collection of these points forms a powerful invariant of $f$ (as we'll see later in this post), especially once we count them using their analog of [multiplicities](https://en.wikipedia.org/wiki/Multiplicity_(mathematics)#Multiplicity_of_a_root_of_a_polynomial). In both the algebraic and analytic settings, this analog is called the *order* of $f$ at a point $P$, and conceptually it exists because a smooth curve locally looks like a line --- essentially a version of the [implicit function theorem](https://en.wikipedia.org/wiki/Implicit_function_theorem#Two_variables_case). Looking at the complex-analytic case first provides an intuitive picture of zeroes and poles through local coordinates:

> **Definition (order of meromorphic function on a Riemann surface).**  
> Let $f$ be a meromorphic function around point $z_0$ in $\mathbb{C}$. Then the *order* of $f$ at $z_0$ is the value $k\in \mathbb{Z}$ such that $f$ has a [Laurent expansion](https://en.wikipedia.org/wiki/Laurent_series)
>
> $$
> f(z) = \sum_{n = k}^\infty a_n (z-z_0)^n,\qquad a_k \neq 0,
> $$
>
> which rewrites to 
>
> $$f(z) = ( z - z_0)^k u(z)$$
>
> for some local holomorphic function $u(z)$ with $u(z_0) \neq 0$. We write $\mathrm{ord}_{z_0}(f) \mathrel{\vcenter{:}}= k$.
>
...
> Let $f$ be a rational function on a smooth curve $X$, and $P$ a (closed) point on $X$. Then there exists a [local parameter](https://en.wikipedia.org/wiki/Local_parameter) $t$ at $P$ such that $f = t^k \cdot u$ for a rational function $u$ defined but not vanishing at $P$. We set $\mathrm{ord}_{P}(f) \mathrel{\vcenter{:}}= k$.
{: .box .definition }
...
> **Example.**  
> Let $P=(x_P, y_P)$ be a nontrivial point on an elliptic curve $E$. Then the vertical line $v_P$ through $P$ has equation $v_P(x,y) = x - x_P$, and
> 
> $$
...
However, this strategy only works when the definition at hand is close to its
geometric origins.
